...
This experiment was created using PsychoPy3 Experiment Builder (v2024.2.4),
    on Tue 12 Nov 2024 12:49:10 PM EST
...
expName = 'SBL_retrieval_exp'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/media/bw-share/experiments/turk-browne_nbt2/lubianiker_nel33/retrieval_exp/SBL_retrieval_exp.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('key_resp_start_2') is None:
        # initialise key_resp_start_2
        key_resp_start_2 = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='key_resp_start_2',
        )
    if deviceManager.getDevice('key_resp_remember') is None:
        # initialise key_resp_remember
        key_resp_remember = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='key_resp_remember',
        )
    ...
    if deviceManager.getDevice('key_resp_context_conf') is None:
        # initialise key_resp_context_conf
        key_resp_context_conf = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='key_resp_context_conf',
        )
    ...
    if deviceManager.getDevice('key_resp_scene_conf') is None:
        # initialise key_resp_scene_conf
        key_resp_scene_conf = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='key_resp_scene_conf',
        )
    ...
    if deviceManager.getDevice('key_resp_continue') is None:
        # initialise key_resp_continue
        key_resp_continue = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='key_resp_continue',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text_instructions_2 = visual.TextStim(win=win, name='text_instructions_2',
        ...
        languageStyle='LTR',
        depth=0.0);
    key_resp_start_2 = keyboard.Keyboard(deviceName='key_resp_start_2')
    ...
    key_resp_remember = keyboard.Keyboard(deviceName='key_resp_remember')
    ...
    key_resp_context_conf = keyboard.Keyboard(deviceName='key_resp_context_conf')
    ...
    slider_scene_conf = visual.Slider(win=win, name='slider_scene_conf',
        ...
        font='Open Sans', labelHeight=0.03,
        flip=False, ori=0.0, depth=-2, readOnly=False)
    key_resp_scene_conf = keyboard.Keyboard(deviceName='key_resp_scene_conf')
    ...
    text_endSet = visual.TextStim(win=win, name='text_endSet',
        ...
        languageStyle='LTR',
        depth=0.0);
    key_resp_continue = keyboard.Keyboard(deviceName='key_resp_continue')
    ...
    df = pd.read_csv('counterbalanced_vars_behav.csv') # match table to experiment
    ...
    # scene_label --> scene name from stimuli list
    stim_list = '/media/bw-share/experiments/turk-browne_nbt2/lubianiker_nel33/retrieval_exp/stimuli/stim_lists/stimuli_list_sub_' + str(subnum) + '_set_' + session + '_run_1.csv'
    df = pd.read_csv(stim_list)
    # get the unique scene_stimulus and their corresponding scene_label
    labels = df[['scene_stimulus', 'scene_label']].drop_duplicates().reset_index(drop=True)
    #  leave in scene_stimulus only the two characters that represent the scene, before the .png
    labels['scene_stimulus'] = labels['scene_stimulus'].str[-6:-4]
    ...
    while continueRoutine:
        ...
        if key_resp_start.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
            # keep track of start time/frame for later
    ...
    while continueRoutine:
        ...
        if key_resp_start_2.status == NOT_STARTED and tThisFlip >= 2.0-frameTolerance:
            # keep track of start time/frame for later
    ...
    for thisRun in runs:
        ...
        for thisShow_rest_screen in show_rest_screen:
            ...
            # if trial has changed, end Routine now
            if isinstance(show_rest_screen, data.TrialHandler2) and thisShow_rest_screen.thisN != show_rest_screen.thisTrial.thisN:
                continueRoutine = False
            ...
            while continueRoutine:
                ...
                if key_resp_run_rest.status == NOT_STARTED and tThisFlip >= 1-frameTolerance:
                    # keep track of start time/frame for later
    ...
    # for local python version, this is the variable to be called for in the text component.
    # change to window.end_inst when running on pavlovoia
    end_inst = 'Congratulations! You are done with the experiment!\nWe thank you for your participation.\n\nPress SPACEBAR and then "OK" in the prompted window to receive a Prolific completion code.'
